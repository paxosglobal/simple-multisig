...
bytes32 constant SALT = 0x251543af6a222378665a76fe38dbceae4871a070b7fdaf5c6c30cf758dc33cc0;
  ...
  constructor(uint threshold_, address[] owners_, uint chainId) public {
    require(owners_.length <= 20 && threshold_ <= owners_.length && threshold_ > 0, "a");

    address lastAdd = address(0);
    for (uint i = 0; i < owners_.length; i++) {
      require(owners_[i] > lastAdd, "b");
  ...
  function execute(uint8[] sigV, bytes32[] sigR, bytes32[] sigS, address destination, uint value, bytes data, address executor, uint gasLimit) public {
    require(sigR.length == threshold, "c");
    require(sigR.length == sigS.length && sigR.length == sigV.length, "d");
    require(executor == msg.sender || executor == address(0), "e");
    ...
    for (uint i = 0; i < threshold; i++) {
      ...
      require(recovered > lastAdd && isOwner[recovered], "f");
      lastAdd = recovered;
    }
